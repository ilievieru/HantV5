...
var started = false;
var type = "LuminositySensor";
var currentLumenValue;
var port = process.argv[2] === undefined ? 8083 : process.argv[2];
var serviceId = 4+port;
...
function generateRawData(){
    if(currentLumenValue === undefined || currentLumenValue < 1 || currentLumenValue > 130)
        currentLumenValue = tools.rand(100);
    else
        currentLumenValue = currentLumenValue * (tools.rand2(7, 13) / 10);

    var LightLevel;

    if(currentLumenValue<=20)
        LightLevel = 0;
    else if(currentLumenValue<=40)
        LightLevel = 1;
    else if(currentLumenValue<=60)
        LightLevel = 2;
